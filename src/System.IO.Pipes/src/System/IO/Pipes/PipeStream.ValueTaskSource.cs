    ...
    public abstract partial class PipeStream : Stream
    {
        internal abstract unsafe class PipeValueTaskSource : IValueTaskSource<int>, IValueTaskSource
        {
            ...
            internal void FinishedScheduling()
            {
                ...
                // to make the call.
                ulong result = (ulong)Interlocked.Exchange(ref Unsafe.As<ulong, long>(ref _result), 1);
                if (result != 0)
                {
            ...
            private static void IOCallback(uint errorCode, uint numBytes, NativeOverlapped* pOverlapped)
            {
                ...
                if (Interlocked.Exchange(ref Unsafe.As<ulong, long>(ref vts._result), (long)((1ul << 63) | ((ulong)numBytes << 32) | errorCode)) != 0)
                {
