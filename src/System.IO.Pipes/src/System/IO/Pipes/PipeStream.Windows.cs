...
namespace Backports.System.IO.Pipes
{
    public abstract partial class PipeStream : Stream
    {
        ...
        public override int Read(byte[] buffer, int offset, int count)
        {
            ...
            this.ValidateBufferArguments(buffer, offset, count);
            if (!CanRead)
            {
        ...
        public ValueTask<int> ReadAsync(Memory<byte> buffer, CancellationToken cancellationToken = default(CancellationToken))
        {
            if (!_isAsync)
            {
                ...
                return new(base.ReadAsync(segment.Array!, segment.Offset, segment.Count, cancellationToken));
            }
            ...
            if (cancellationToken.IsCancellationRequested)
            {
                return ValueTaskHelpers.FromCanceled<int>(cancellationToken);
            }
        ...
        public ValueTask WriteAsync(ReadOnlyMemory<byte> buffer, CancellationToken cancellationToken = default(CancellationToken))
        {
            if (!_isAsync)
            {
                ...
                return new(base.WriteAsync(segment.Array!, segment.Offset, segment.Count, cancellationToken));
            }
            ...
            if (cancellationToken.IsCancellationRequested)
            {
                return ValueTaskHelpers.FromCanceled(cancellationToken);
            }
                ...
                if (Interop.Kernel32.ReadFile(_handle!, (byte*)vts._memoryHandle.Pointer, buffer.Length, IntPtr.Zero, vts._overlapped) == 0)
                {
                    ...
                    switch (errorCode)
                    {
                        ...
                        default:
                            ...
                            vts.Dispose();
                            return ValueTaskHelpers.FromException<int>(Win32Marshal.GetExceptionForWin32Error(errorCode));
                    }
                }
            }
            catch
                ...
                if (Interop.Kernel32.WriteFile(_handle!, (byte*)vts._memoryHandle.Pointer, buffer.Length, IntPtr.Zero, vts._overlapped) == 0)
                {
                    ...
                    switch (errorCode)
                    {
                        ...
                        default:
                            ...
                            vts.Dispose();
                            return ValueTaskHelpers.FromException(ExceptionDispatchInfoHelpers.SetCurrentStackTrace(WinIOError(errorCode)));
                    }
                }
            }
            catch
